...
/// @brief会根据suggestedFilename(为空prefix[%3d].suffix)形式生成唯一文件返回
/// @brief如果目录转化失败会在suggestedPath(为空取app->doc)目录下生成
...
+ (NSString*) absolutePath:(NSString*)path
         suggestedFilename:(NSString*)suggestedFilename
             ...
             allowSameName:(BOOL)allowSameName
                   context:(PDRCoreApp*)context;
/// @brief转化H5+路径到系统路径如果路径是目录
/// @brief会根据prefix[%3d].suffix形式生成唯一文件返回
/// @brief如果目录转化失败会在app->doc目录下生成
